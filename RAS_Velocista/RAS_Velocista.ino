...
* 
** Fuente: http://robots-argentina.com.ar/didactica/puente-h-placa-controladora-de-motores-l9110s/
*/


const byte  VelMAX  = 255;    //Velocidad maxima de robot
...
void loop() {
  ...
  if(!valorIzq&&valorDer){    //salio por izquierda
    fueHaciaIzquierda = 1;
  }
  if(valorIzq&&!valorDer){
    fueHaciaDerecha = 1;      //salio por derecha
  }

  if(fueHaciaIzquierda){
    if(!valorIzq&&valorDer){
      GiroDerechaVel(250,200);
    }
    if(valorIzq&&valorDer){
      GiroDerecha();
    }
